...
class AgendamentoParser {
    ...
    parseMultiple(content) {
        ...
        // Estratégia 1: Dividir por múltiplas linhas vazias (bloco separator)
        // Procura por "Horário:" como identificador de bloco
        ...
        blocos.forEach((bloco, index) => {
            // Reconstrói o bloco adicionando "Horário:" de volta
            ...
            if (agendamento) {
                ...
                console.log(`✅ Agendamento ${index + 1} parseado: ${agendamento.horario} - ${agendamento.paciente} - R$ ${agendamento.valorAtendimento.toFixed(2)}`);
            } else {
                console.warn(`⚠️ Bloco ${index + 1} não pôde ser processado`);
            }
        });
        
        console.log(`📊 Total de agendamentos processados com sucesso: ${agendamentos.length}`);
        return agendamentos;
    }
